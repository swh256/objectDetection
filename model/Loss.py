...
class MyLoss(nn.Cell):
    ...
    def construct(self, pred, labels):
        """
        :param pred: (batchsize,11,7,7)的网络输出数据
            ...
            for n in range(7):  # x方向网格循环
                for m in range(7):  # y方向网格循环
                    if labels[i, 4, m, n] == 1:  # 如果包含物体
                        ...
                        # 选择iou大的bbox作为负责物体
                        if iou1 >= iou2:
                            coor_loss = coor_loss + 5 * (((pred[i, 0:2, m, n] - labels[i, 0:2, m, n])**2).sum()
                                                         + ((np.sqrt(pred[i, 2:4, m, n])-np.sqrt(labels[i, 2:4, m, n]))**2).sum())
                            obj_confi_loss = obj_confi_loss + \
                                (pred[i, 4, m, n] - iou1)**2
                            # iou比较小的bbox不负责预测物体，因此confidence loss算在noobj中，注意，对于标签的置信度应该是iou2
                            noobj_confi_loss = noobj_confi_loss + \
                                0.5 * ((pred[i, 9, m, n]-iou2)**2)
                        else:
                            coor_loss = coor_loss + 5 * (((pred[i, 5:7, m, n] - labels[i, 5:7, m, n])**2).sum()
                                                         + ((np.sqrt(pred[i, 7:9, m, n])-np.sqrt(labels[i, 7:9, m, n]))**2).sum())
                            obj_confi_loss = obj_confi_loss + \
                                (pred[i, 9, m, n] - iou2)**2
                            # iou比较小的bbox不负责预测物体，因此confidence loss算在noobj中,注意，对于标签的置信度应该是iou1
                            noobj_confi_loss = noobj_confi_loss + \
                                0.5 * ((pred[i, 4, m, n]-iou1) ** 2)
                        class_loss = class_loss + \
                            ((pred[i, 10:, m, n] - labels[i, 10:, m, n])
                             ** 2).sum()
                    else:  # 如果不包含物体
                        noobj_confi_loss = noobj_confi_loss + 0.5 * \
                            (pred[i, [4, 9], m, n]**2).sum()
